      ...
      <div className="w-3/4 m-auto">
        <Slider {...settings} ref={sliderRef}>
          {SERVICES.map((service) => (
            <div
              className="bg-white text-black rounded-xl flex flex-row justify-center items-center !flex !flex-row"
              key={service.name}
            >
              <div className="rounded-t-xl w-1/2">
                <img src={service.image} alt="services_image" className="w-80" />
              </div>

              <div className="w-1/2 flex flex-col gap-6">
                <h2 className="font-bold text-4xl">{service.name}</h2>
                <p className="font-extralight items-center justify-center text-justify">{service.description}</p>
